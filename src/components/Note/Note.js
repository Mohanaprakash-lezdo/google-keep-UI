...
//   const [editedTitle, setEditedTitle] = useState("");
//   const [editedContent, setEditedContent] = useState("");
//   const [editedImage, setEditedImage] = useState(null); // ✅ Added state for edited image
...
//     setEditedTitle(title);
//     setEditedContent(content);
//     setEditedImage(image); // ✅ Preserve image when editing
//     setIsEditing(true);
//   };
...
//       reader.onloadend = () => {
//         setEditedImage(reader.result); // ✅ Store base64 image
//       };
...
//   // Save changes
//   const handleSave = () => {
//     dispatch(updateNote({ id, title: editedTitle, content: editedContent, image: editedImage }));
//     setIsEditing(false);
//   };
...
//   // ✅ Toggle Pin/Unpin Note Correctly
//   const handlePin = (e) => {
//     e.stopPropagation(); // Prevent modal from opening
//     if (isPinned) {
//       dispatch(unpinNote({ id })); // ✅ Unpin note
...
//       {showMenu && (
//         <div className="menu">
//           <button onClick={() => dispatch(copyNote({ id, title, content, image }))}>
//             <ContentCopyIcon /> Copy
//           </button>
...
const Note = ({ id }) => {
  ...
  const handleSave = () => {
    ...
    setIsEditing(false);
    setIsExpanded(false); // Close modal after saving
  };
  ...
  return (
    ...
    >
      <div className="note-content" onClick={handleExpand}>
        <h2>{title}</h2>
        <p>{content}</p>
        {image && <img src={image} alt="Note Attachment" className="note-image" />}
      </div>
      ...
      {hovered && (
        <div className="icons">
          {!isArchived && (
            <button className="pin-icon" title="Pin Note">
              <PushPinIcon style={{ color: isPinned ? "black" : "grey" }} />
            </button>
          )}
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            {!isEditing ? (
              ...
              <>
                ...
                />
                <input type="file" accept="image/*" onChange={handleImageUpload} />
                {editedImage && <img src={editedImage} alt="Preview" className="note-image" />}
                <button onClick={handleSave}>Save</button>
                <button onClick={() => setIsEditing(false)}>Cancel</button>
              </>
            )}
